...
//    }
    ...
    //重建索引
//    auto tabs = this->sm_manager_->db_.get_tables();
    for (auto &iter: this->sm_manager_->db_.get_tables()) {
        ...
        for (auto &indexe: iter.second.indexes) {
            tmp_indexes.emplace_back(indexe);
        }
        for (auto &tmp_indexe: tmp_indexes) {
            sm_manager_->drop_index(tmp_indexe.tab_name, tmp_indexe.cols, nullptr);
            std::vector<std::string> col_names;
            for (auto &col: tmp_indexe.cols) {
                col_names.emplace_back(col.name);
            }
            sm_manager_->create_index(tmp_indexe.tab_name, col_names, nullptr);
        }
    }
