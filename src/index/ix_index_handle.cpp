...
std::pair<IxNodeHandle *, bool> IxIndexHandle::find_leaf_page(const char *key, Operation operation,
    ...
    while (!cur_node_hdl->is_leaf_page()) {
        page_id_t child_page_no = cur_node_hdl->internal_lookup(key);
        buffer_pool_manager_->unpin_page(cur_node_hdl->get_page_id(), false);
        delete cur_node_hdl;
        cur_node_hdl = fetch_node(child_page_no);
    }

    // 3. 找到包含该key值的叶子结点停止查找，并返回叶子节点
    return std::make_pair(cur_node_hdl, find_first);
}
...
IxNodeHandle *IxIndexHandle::split(IxNodeHandle *node) {
    ...
    //需要初始化新节点的page_hdr内容
    newNode->page_hdr->num_key = 0;
    newNode->page_hdr->is_leaf = node->page_hdr->is_leaf;
    newNode->page_hdr->parent = node->get_parent_page_no();
    newNode->page_hdr->next_free_page_no = node->page_hdr->next_free_page_no;
    // 2. 如果新的右兄弟结点是叶子结点，更新新旧节点的prev_leaf和next_leaf指针
    if (newNode->is_leaf_page()) {
        newNode->page_hdr->prev_leaf = node->get_page_no();
        newNode->page_hdr->next_leaf = node->get_next_leaf();
        node->page_hdr->next_leaf = newNode->get_page_no();

        //    为新节点分配键值对，更新旧节点的键值对数记录
        // 如果新的右兄弟节点不是叶子结点，就获取新节点的后继节点
        IxNodeHandle *next_node = fetch_node(newNode->page_hdr->next_leaf);
        // 更新新节点的后继节点
        next_node->page_hdr->prev_leaf = newNode->get_page_no();
    ...
    newNode->insert_pairs(0, node->get_key(pos), node->get_rid(pos), num);
...
bool IxIndexHandle::delete_entry(const char *key, Transaction *transaction) {
    ...
    // 3. 如果删除成功需要调用CoalesceOrRedistribute来进行合并或重分配操作，并根据函数返回结果判断是否有结点需要删除
    coalesce_or_redistribute(target_node_hdl, transaction, &is_root_latched);
...
bool IxIndexHandle::coalesce_or_redistribute(IxNodeHandle *node, Transaction *transaction, bool *root_is_latched) {
    ...
    if (node->is_root_page()) {
        ...
        if (node->get_size() >= node->get_min_size()) {
            ...
            // 3. 寻找node结点的兄弟结点（优先选取前驱结点）
            IxNodeHandle *neighbor_node_hdl = fetch_node(
                    (index == 0) ? parent_node_hdl->value_at(1) : parent_node_hdl->value_at(index - 1));
            // 4. 如果node结点和兄弟结点的键值对数量之和，能够支撑两个B+树结点（即node.size+neighbor.size >=
            // NodeMinSize*2)，则只需要重新分配键值对（调用Redistribute函数）
            if (neighbor_node_hdl->get_size() + node->get_size() >= node->get_min_size() * 2) {
                ...
                coalesce(&neighbor_node_hdl, &node, &parent_node_hdl, index, transaction, root_is_latched);
                buffer_pool_manager_->unpin_page(parent_node_hdl->get_page_id(), true);
                buffer_pool_manager_->unpin_page(neighbor_node_hdl->get_page_id(), true);
